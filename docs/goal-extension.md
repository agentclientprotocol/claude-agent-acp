...
Some Claude runtime versions persist the goal and begin the Stop-hook workflow without emitting the initial `active_goal` message on the SDK stream. To avoid leaving clients without visible state for the whole long-running prompt, the adapter publishes a minimal active snapshot as soon as it submits `/goal <condition>`, and publishes `goal: null` for `/goal clear`. A later runtime `active_goal` remains authoritative and replaces that optimistic snapshot with iteration, reason, and timestamp fields or clears it.

Only `clear` is advertised for out-of-band control. When a goal turn is running, the adapter injects `/goal clear` at steering priority `now` so the command cannot remain queued behind the Stop-hook workflow; on an idle session it uses the ordinary prompt lifecycle. Setting a goal remains available through `/goal <condition>` and is not advertised as an extension action because it starts Claude's long-running Stop-hook workflow. Cancelling a turn does not clear the session-scoped goal: clients use the goal control method when they intend to remove it.
